...
$(document).ready(function () { //kiedy (DOM) sie załaduje
    checkScroll(); // sprawdza czy strona sie skroluje
    $(window).scroll(checkScroll); //zwraca pozycje skrola
});

function checkScroll() { //sprawdza czy strona sie skroluje
if ($(window).scrollTop() >= 300) { //jezeli jest skrolowana na wiecej niz 300 pixeli
    $('.navbar').addClass('solid'); //to dodaj 'solid' na elementy 'navbar'
} else { //a jezeli nie jest
    $('.navbar').removeClass('solid'); // usun solid z elementow navbar
}
}
...
$('.navbar-toggler').click(function () { // kiedy navbar jest klikniety
    if ($(window).scrollTop() <= 300) { // jezeli jest skrolowane
    $("nav.navbar").toggleClass("solid-toggle"); // dodaj solid do navbar
    }
});


/*========== CLOSE MOBILE MENU ON CLICK & SMOOTH SCROLL TO LINK a[href^="#"] ==========*/
// przekierowanie po kliknięciu + czyste najechanie
